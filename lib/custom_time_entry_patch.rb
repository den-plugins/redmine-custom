        ...
        if hours.present?
          ...
          if project.project_type.scan(/^(Admin)/).flatten.present?
            ...
            if membership=project.members.project_team.detect {|m| m.user_id == user_id}
              ...
              # non-billable tasks are loggable by allocated users (shadows, billable and non-billable users)
              # billable tasks are loggable only by billable users
              errors.add_to_base l(:error_timelog_project_allocation) if issue_is_billable && !member_is_billable
            else
              errors.add_to_base l(:error_timelog_project_membership)
              errors.add_to_base l(:error_timelog_project_allocation)
            end
