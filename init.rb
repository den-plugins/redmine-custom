if Rails::VERSION::MAJOR >= 3
  ActionDispatch::Callbacks.to_prepare do
    require_dependency 'accounting_type'
    # Model Patches
    Project.send(:include, Custom::AccountingPatch)
    Issue.send(:include, Custom::AccountingPatch, Custom::IssuePatch)
    # Controller Patches
    IssuesController.send(:include, Custom::IssuesControllerPatch)
    ProjectsController.send(:include, Custom::ProjectsControllerPatch)
    ContextMenusController.send(:include, Custom::ContextMenusControllerPatch)
    # Helper Patches
    IssuesHelper.send(:include, Custom::IssuesHelperPatch)
  end
else
  require 'redmine'
  require 'dispatcher'
...
Redmine::Plugin.register :redmine_custom do
  ...
  version '1.0.0'
  author_url 'http://exist.com/about'
end
