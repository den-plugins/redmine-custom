...
class IssuesController < ApplicationController
  ...
  def sort_by
    ...
    if request.xhr?
      render :update do |page|
        # it only work for now on the hard coded partial_name
        # TODO:must find a way to pass the partial name from the context menu
        page.replace_html 'timelogging', :partial =>  'my/blocks/timelogging' ,:locals => {:user => User.current}
        page.visual_effect :highlight, 'timelogging'
      end
    end
  end
  ...
  def destroy
    @hours = TimeEntry.sum(:hours, :conditions => ['issue_id IN (?)', @issues]).to_f unless params[:children_todo]
    if @hours > 0
      case params[:todo]
      when 'destroy'
        # nothing to do
  ...
  def preview
    ...
    user_action = params[:user_action] || 'new'
    case user_action
      when /new/
 ...
 # REST api for Performing ticket task by remote

  #add a new issue by remote
  def new_remote
    respond_to do |accepts|
      accepts.html{}
      accepts.xml {
          interface_token = params[:interface_token]
          if request.post? and interface_token == AUTH_TOKEN
              ...
              tracker     = Tracker.find_by_id(tracker_id) #1-bug, #2-feature, #3-support
              project     = Project.find_by_name(proj_name) #specify the name of the project
              status      = IssueStatus.find_by_id(status_id)# 1 indicates a new ticket
              author      = User.find_by_id(author_id) #this is the id no. assigned for the redmine user
              #use 73 for official appspace, 2 for local


              @issue = Issue.new(:tracker_id   => tracker.id,
                                 :subject      => subject,
                                 :description  => description,
                                 :project_id   => project.id,
                                 :status_id    => status.id,
                                 :priority_id  => priority_id, #4 is normal, #7 is immediate, 6 is urgent
                                 :author_id    => author.id
                                )
            ...
            if @issue.save
                ...
                logger.info "Issue succefully saved!"
                logger.info "Inspect #{@issue.inspect}"
                # this will update the status
                if status.id != 1
                  update_status_remote(status_id,@issue.id,true)
                end
            else
                logger.info "Failed to save issue"
            end

            ticket = {'ticket_id' => @issue.id}

            render :xml => ticket.to_xml
        else
            logger.error "Request sent is not via POST"
        end
     }
    end
    ...
    respond_to do |accepts|
      ...
      accepts.xml {
          ...
          if request.post? and interface_token == AUTH_TOKEN
             ...
             @user        = User.find_by_id(user_id) #use the mystic account 73
             @issue       = Issue.find_by_id(issue_id)
             update_notes = message
             journal      = @issue.init_journal(@user,update_notes)

             if journal.save
             #------ Call the Update Status Method -----#"
                if @issue.status_id != status_id #it means the status has changed and needs to be updated
                  update_status_remote(status_id,issue_id,true)
                end
                logger.info "::::Journal Update successfully saved::::"
             else
                logger.info "::::Updates were not saved::::"
             end
 ...
 def update_status_remote(status_id = nil, issue_id = nil, from_controller = false)

   unless from_controller
     respond_to do |accepts|
       accepts.xml{
         interface_token = params[:interface_token]
         if interface_token == AUTH_TOKEN && request.post?
           status_id = params[:status_id]
  ...
  #-- after obtaining status id either from interface or from controller --#
   case status_id.to_i
     when 2 then redmine_status_id = 5 #close issue
     when 3 then redmine_status_id = 7 #in progress
     when 4 then redmine_status_id = 1 #reopen/make the status  new again
   else
      redmine_status_id = 1
   end
   @issue = Issue.find_by_id(issue_id)
   @issue.update_attributes(:status_id => redmine_status_id)
end
